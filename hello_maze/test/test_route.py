...
class TestRoute(unittest.TestCase):
    ...
    def test_apply_short_cuts(self):
        test_positions = self.get_test_positions(nr=5)
        ...
        route.append(test_positions[2][0])
        route.append(test_positions[3][0])
        route.append(test_positions[4][0])

        route.apply_short_cuts()
        self.assertEqual(route.length, 5,"Apply short cuts does not change route with no short cuts")
        ...
        route.append(test_positions[3][0])
        route.append(test_positions[4][0])
        route.append(test_positions[4][1])
        route.append(test_positions[3][1])
        route.append(test_positions[2][1])


        route.apply_short_cuts()
        self.assertEqual(route.length, 4,"Apply short cuts reduces for neighbor further in the route")

        route_zigzag = Route()
        route_zigzag.append(test_positions[0][0])
        route_zigzag.append(test_positions[0][1])
        route_zigzag.append(test_positions[0][2])
        route_zigzag.append(test_positions[0][3])
        route_zigzag.append(test_positions[0][4])
        route_zigzag.append(test_positions[1][4])
        route_zigzag.append(test_positions[2][4])
        route_zigzag.append(test_positions[3][4])
        route_zigzag.append(test_positions[4][4])
        route_zigzag.append(test_positions[4][3])
        route_zigzag.append(test_positions[3][3])
        route_zigzag.append(test_positions[2][3])
        route_zigzag.append(test_positions[1][3])
        route_zigzag.append(test_positions[1][2])
        route_zigzag.append(test_positions[2][2])
        route_zigzag.append(test_positions[3][2])
        route_zigzag.append(test_positions[4][2])
        route_zigzag.append(test_positions[4][1])
        route_zigzag.append(test_positions[3][1])
        route_zigzag.append(test_positions[2][1])
        route_zigzag.append(test_positions[1][1])
        route_zigzag.append(test_positions[1][0])
        route_zigzag.append(test_positions[2][0])
        route_zigzag.append(test_positions[3][0])
        route_zigzag.append(test_positions[4][0])

        route = route_zigzag.copy()
        route.apply_short_cuts()
        logging.debug(f"Route after zigzag short cuts= {str(route)}")
        self.assertEqual(route.length, 5,"Apply short cuts reduces zig zag route to 5")

        route = route_zigzag.get_sub_route(test_positions[4][0], test_positions[0][4])
        logging.debug(f"Sub route before zigzag short cuts= {str(route)}")
        route.apply_short_cuts()
        logging.debug(f"Sub route after zigzag short cuts= {str(route)}")
        self.assertEqual(route.length, 9,"Apply short cuts reduces zig zag route to 9")

        route = route_zigzag.get_sub_route(test_positions[4][0], test_positions[0][3])
        logging.debug(f"Sub route before zigzag short cuts= {str(route)}")
        route.apply_short_cuts()
        logging.debug(f"Sub route after zigzag short cuts= {str(route)}")
        self.assertEqual(route.length, 8,"Apply short cuts reduces zig zag route to 9")